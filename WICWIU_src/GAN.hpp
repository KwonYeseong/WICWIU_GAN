...
template<typename DTYPE> int GAN<DTYPE>::TrainGeneratorOnGPU(){
        ...
        for(int ba = 0; ba < 1; ba++){
            std::cout << "Gen Fake Forward ba = : " << ba << " ===== " << (*m_pDiscriminator->GetResult()->GetResult())[Index5D(m_pDiscriminator->GetResult()->GetResult()->GetShape(), 0, ba, 0, 0, 0)] << '\n';
        }
...
template<typename DTYPE> int GAN<DTYPE>::TrainDiscriminatorOnGPU(){
        ...
        for(int ba = 0; ba < 1; ba++){
            std::cout << "Dis True Forward ba = : " << ba << " ===== " << (*m_pDiscriminator->GetResult()->GetResult())[Index5D(m_pDiscriminator->GetResult()->GetResult()->GetShape(), 0, ba, 0, 0, 0)] << '\n';
        }
        ...
        this->ForwardPropagateOnGPU();
        for(int ba = 0; ba < 1; ba++){
            std::cout << "Dis Fake Forward ba = : " << ba << " ===== " << (*m_pDiscriminator->GetResult()->GetResult())[Index5D(m_pDiscriminator->GetResult()->GetResult()->GetShape(), 0, ba, 0, 0, 0)] << '\n';
        }
        m_aDiscriminatorLossFunction->ForwardPropagateOnGPU();
        m_aDiscriminatorLossFunction->BackPropagateOnGPU();
