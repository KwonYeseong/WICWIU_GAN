...
#define REAL_NUMBER_OF_CLASS          5749
...
private:
    string m_path             = "./data";
    string m_dirOfTrainImage  = "lfw_funneled";
    string m_dirOfTestImage   = "lfw_funneled";
    string m_classInformation = "synset_word.txt";
    ...
    queue<int> m_selectedimgNumForAll;
...
public:
    ...
    int CheckClassList() {
        ...
        if (pFile == NULL) {
            printf("In CheckClass file open fail\n");
            exit(-1);
        } else {
            char realValue[20];
    ...
    int CreateImageListOfEachClass() {
        ...
        if (m_isTrain) {
            ...
            for (int classNum = 0; classNum < NUMBER_OF_CLASS; classNum++) {
                sprintf(filePath, "%s/%s/%s/list.txt", m_path.c_str(), m_dirOfTrainImage.c_str(), m_className[classNum].c_str());
                // const char *cstr = filePath.c_str();
                ...
                if (pFile == NULL) {
                    printf("In CreateImageList1 file open fail\n");
                    exit(-1);
                } else {
                    char realValue[100];
            ...
            for (int classNum = 0; classNum < NUMBER_OF_CLASS; classNum++) {
                ...
                if (pFile == NULL) {
                    printf("In CreateImageList2 file open fail\n");
                    exit(-1);
                } else {
                    char realValue[256];
            ...
            for (int classNum = 0; classNum < NUMBER_OF_CLASS; classNum++) {
                sprintf(filePath, "%s/%s/%s/list.txt", m_path.c_str(), m_dirOfTestImage.c_str(), m_className[classNum].c_str());
                ...
                if (pFile == NULL) {
                    printf("In CreateImageList test file open fail\n");
                    exit(-1);
                } else {
                    char realValue[256];
                    ...
                    if (fscanf(pFile, "%s", realValue)) {
                        ...
                        m_numOfTestImage += numOfImageOfClass;
                        // std::cout << m_aNumOfImageOfClass[i] << '\n';
        ...
        if (m_isTrain) {
            ...
            do {
                ...
                for (int i = 0; i < m_batchSize; i++) {
                    ...
                    m_selectedimgNumForAll.push(imgNum);
    ...
    int Data2Tensor() {
        ...
        if (m_isTrain) {
            do {
                ...
                imgNum = m_selectedimgNumForAll.front();
                m_selectedimgNumForAll.pop();

                sem_post(&m_mutexForSelectedDataInformation);
                sem_post(&m_emptyForSelectedDataInformation);

                // 안에서 막아줄 필요가 있다.
                preprocessedImages = this->Image2Tensor(m_aLable[imgNum], imgNum);
                preprocessedLabels = this->Label2Tensor(m_aLable[imgNum]);
    ...
    void Tensor2Image(Tensor<DTYPE> *temp, const char *FILENAME, int colorDim, int height, int width) {
        ...
        if (!(jpegFile = fopen(temp_name_re, "wb"))) {
            printf(" Tensor2Image file open fail\n");
            exit(-1);
        }
